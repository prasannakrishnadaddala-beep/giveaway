...
Payment: PhonePe / UPI QR (manual admin confirmation)
Flow:
  1. Customer clicks Buy → sees PhonePe QR + exact amount + order ref
  2. Customer pays via PhonePe / GPay / Paytm / any UPI app
  3. Customer enters their 12-digit UTR / transaction ID
  4. Admin sees pending queue → clicks Confirm (or Reject)
  5. Ticket activates → slots fill → draw triggers automatically if full
  6. Admin manually transfers refund to winner via UPI & marks done
"""
...
# ── UPI / PhonePe config ──────────────────────────────────────────────────────
UPI_ID    = os.environ.get("UPI_ID",   "yourupi@ybl")    # set in Railway env vars
UPI_NAME  = os.environ.get("UPI_NAME", "LuckyCart")
UPI_QR_URL= os.environ.get("UPI_QR_URL", "")            # optional: your hosted QR image
...
SCHEMA = [
    ...
    """CREATE TABLE IF NOT EXISTS products (
        ...
        original_price INTEGER NOT NULL, ticket_price INTEGER NOT NULL,
        total_slots INTEGER NOT NULL, filled_slots INTEGER DEFAULT 0,
        status TEXT DEFAULT 'active', draw_seed TEXT, winner_user_id INTEGER,
        created_at TIMESTAMPTZ DEFAULT NOW(), drawn_at TIMESTAMPTZ
    )""",
    ...
    """CREATE TABLE IF NOT EXISTS draws (
        id SERIAL PRIMARY KEY, product_id INTEGER NOT NULL,
        winner_ticket_id INTEGER, winner_user_id INTEGER,
        seed TEXT NOT NULL, winning_slot INTEGER NOT NULL,
        total_slots INTEGER NOT NULL, refund_amount INTEGER NOT NULL,
        refund_status TEXT DEFAULT 'pending', refund_upi_ref TEXT,
...
def upi_link(amount_paise, ref):
    """Opens PhonePe / GPay / Paytm directly on mobile with prefilled amount."""
...
def run_draw(product_id):
    ...
    try:
        ...
        cur.execute(
            "SELECT t.*,u.name AS uname,u.email AS uemail,u.phone AS uphone "
        ...
        if not slots: return {"error":"No paid tickets"}
        ...
        refund = cur.fetchone()["tot"]

        cur.execute(
            "INSERT INTO draws (product_id,winner_ticket_id,winner_user_id,seed,winning_slot,total_slots,refund_amount) "
            "VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING id",
            (product_id,winner["id"],winner["user_id"],seed,winning_slot,len(slots),refund)
        )
        draw_id = cur.fetchone()["id"]
        cur.execute("UPDATE products SET status='drawn',draw_seed=%s,winner_user_id=%s,drawn_at=NOW() WHERE id=%s",
                    (seed,winner["user_id"],product_id))
        cur.execute("UPDATE tickets SET is_winner=TRUE WHERE id=%s",(winner["id"],))
        conn.commit()

        log.info("Draw done product=%s winner=%s refund=₹%.2f",product_id,winner["user_id"],refund/100)
        return {"draw_id":draw_id,"winner_name":winner["uname"],"winner_email":winner["uemail"],
                "winner_phone":winner.get("uphone",""),"winning_slot":winning_slot,
                "total_slots":len(slots),"seed":seed,"refund_amount":refund}
    except Exception as e:
...
def register():
    ...
    if not name or not email or not pwd: return jsonify({"error":"Name, email, password required"}),400
    if len(pwd)<8: return jsonify({"error":"Password min 8 chars"}),400
    if query("SELECT id FROM users WHERE email=%s",(email,),one=True): return jsonify({"error":"Email already registered"}),409
    conn=get_db(); cur=conn.cursor()
    cur.execute("INSERT INTO users (name,email,phone,password,is_admin) VALUES (%s,%s,%s,%s,%s) RETURNING id,is_admin",
...
def login():
    ...
    if not user or not check_password_hash(user["password"],d.get("password","")): return jsonify({"error":"Invalid credentials"}),401
...
def create_product():
    ...
    conn=get_db(); cur=conn.cursor()
    cur.execute("INSERT INTO products (name,description,category,emoji,original_price,ticket_price,total_slots) VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING id",
...
def update_product(pid):
    ...
    vals.append(pid); query(f"UPDATE products SET {', '.join(sets)} WHERE id=%s",vals,commit=True)
    return jsonify({"message":"Updated"})

# ═══════════════════════════════════════════════════════════════════════════════
#  PAYMENT routes — UPI / PhonePe
# ═══════════════════════════════════════════════════════════════════════════════

@app.route("/api/orders/create",methods=["POST"])
@login_required
def create_order():
    """Step 1: Reserve ticket, return UPI payment details to show customer."""
    d=request.json or {}
    ...
    conn=get_db(); cur=conn.cursor()
    cur.execute("INSERT INTO tickets (product_id,user_id,quantity,amount_paid,order_ref,payment_status) VALUES (%s,%s,%s,%s,%s,'pending') RETURNING id",
...
@login_required
def submit_utr():
    """Step 2: Customer submits their UPI UTR after paying."""
    d=request.json or {}
    tid=d.get("ticket_id"); utr=d.get("utr","").strip()
    if not utr or len(utr)<6: return jsonify({"error":"Enter a valid UTR / transaction ID"}),400
    conn=get_db(); cur=conn.cursor()
    ...
    conn.commit(); conn.close()
    return jsonify({"message":"Submitted! Your ticket activates once we verify your payment (within 30 min).","ticket_id":tid})

@app.route("/api/admin/confirm-payment/<int:tid>",methods=["POST"])
@login_required
@admin_required
def confirm_payment(tid):
    """Admin confirms UPI payment → ticket goes paid → slots fill → draw if full."""
    conn=get_db(); cur=conn.cursor()
    ...
    cur.execute("UPDATE products SET filled_slots=filled_slots+%s WHERE id=%s RETURNING filled_slots,total_slots,id",
    ...
    return jsonify({"message":"Payment confirmed! Ticket activated.","filled_slots":prod["filled_slots"],"total_slots":prod["total_slots"],"draw_result":draw_result})
...
@app.route("/api/admin/mark-refunded/<int:draw_id>",methods=["POST"])
@login_required
@admin_required
def mark_refunded(draw_id):
    d=request.json or {}; ref=d.get("upi_ref","")
    query("UPDATE draws SET refund_status='processed',refund_upi_ref=%s WHERE id=%s",(ref,draw_id),commit=True)
    query("UPDATE tickets SET payment_status='refunded' WHERE product_id=(SELECT product_id FROM draws WHERE id=%s) AND user_id=(SELECT winner_user_id FROM draws WHERE id=%s)",(draw_id,draw_id),commit=True)
    return jsonify({"message":"Refund marked sent."})
...
@login_required
def my_tickets():
    rows=query("SELECT t.*,p.name AS pname,p.emoji,p.category,p.original_price,p.status AS pstatus FROM tickets t JOIN products p ON p.id=t.product_id WHERE t.user_id=%s ORDER BY t.created_at DESC",(session["user_id"],))
    return jsonify([dict(r) for r in rows])

@app.route("/api/my/wins")
@login_required
def my_wins():
    rows=query("SELECT d.*,p.name AS pname,p.emoji,p.original_price FROM draws d JOIN products p ON p.id=d.product_id WHERE d.winner_user_id=%s ORDER BY d.drawn_at DESC",(session["user_id"],))
    return jsonify([dict(r) for r in rows])
...
@app.route("/api/draws")
def draw_history():
    rows=query("SELECT d.id,d.drawn_at,d.winning_slot,d.total_slots,d.seed,d.refund_amount,d.refund_status,p.name AS pname,p.emoji,u.name AS wname FROM draws d JOIN products p ON p.id=d.product_id JOIN users u ON u.id=d.winner_user_id ORDER BY d.drawn_at DESC LIMIT 50")
    return jsonify([dict(r) for r in rows])
...
def verify_draw(did):
    ...
    return jsonify({"draw_id":did,"seed":draw["seed"],"hash":h,"computed_slot":cs,"recorded_slot":draw["winning_slot"],"is_valid":cs==draw["winning_slot"]})
...
def pending_payments():
    rows=query("SELECT t.*,u.name AS uname,u.email,u.phone,p.name AS pname,p.emoji,p.ticket_price FROM tickets t JOIN users u ON u.id=t.user_id JOIN products p ON p.id=t.product_id WHERE t.payment_status IN ('utr_submitted','pending') ORDER BY t.created_at DESC")
    return jsonify([dict(r) for r in rows])
...
def admin_all_tickets():
    rows=query("SELECT t.*,u.name AS uname,u.email,p.name AS pname,p.emoji FROM tickets t JOIN users u ON u.id=t.user_id JOIN products p ON p.id=t.product_id ORDER BY t.created_at DESC LIMIT 300")
    return jsonify([dict(r) for r in rows])
